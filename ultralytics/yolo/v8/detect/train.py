...
class Loss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        self.use_nwd = m.use_nwd
        self.nwd_ratio = m.nwd_ratio
        self.nwd_eps = m.nwd_eps
        self.nwd_constant = m.nwd_constant

        self.device = device

        self.assigner = TaskAlignedAssigner(topk=10, num_classes=self.nc, alpha=0.5, beta=6.0)
        self.bbox_loss = BboxLoss(m.reg_max - 1, use_dfl=self.use_dfl, use_nwd=self.use_nwd, nwd_ratio=self.nwd_ratio, eps=self.nwd_eps, constant=self.nwd_constant).to(device)
